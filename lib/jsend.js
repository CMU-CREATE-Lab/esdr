 ...
 */
response.jsendClientError = function(data, httpStatus) {
   httpStatus = httpStatus || 400;
   return this.status(httpStatus).json({
                                          code : httpStatus,
                                          status : 'fail',   // seems counterintuitive, but that's what JSend calls for
                                          data : data
                                       });
};
...
response.jsendServerError = function(message, data, httpStatus) {
   ...
   return this.status(httpStatus).json({
                                          code : httpStatus,
                                          status : 'error',  // seems counterintuitive, but that's what JSend calls for
                                          data : data,
                                          message : message
                                       });
};
 ...
 *
 * @param jsendResponse The JSend response to pass through
 */
