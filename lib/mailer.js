...
const log = require('log4js').getLogger('esdr:lib:mailer');
...
const createSenderFromClient = function(client) {
   return {
...
module.exports = {
   ...
   sendVerificationEmail : function(client, recipientEmail, verificationToken) {
      client = client || config.get("esdrClient");
      // send the email later
      setImmediate(function() {
         const sender = createSenderFromClient(client);

         // build the verification URL, inserting the token into it
         let verificationUrl = client.verificationUrl || "";
   ...
   sendPasswordResetEmail : function(client, recipientEmail, resetPasswordToken) {
      ...
      setImmediate(function() {
         ...
         let resetPasswordUrl = client.resetPasswordUrl || "";
